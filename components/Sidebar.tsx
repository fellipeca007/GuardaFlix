...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, setView }) => {
  const getIcon = (id: string, active: boolean) => {
    ...
    // Saved / Bookmark Icon (Moved down)
    if (id === 'SAVED') {
      return active ? (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
    ...
    export const Sidebar: React.FC<SidebarProps> = ({ currentView, setView }) => {
      ...
      return (
        <div className="hidden lg:block w-72 fixed left-0 top-16 bottom-0 p-4 overflow-y-auto bg-slate-900 text-white">
            ...
            {menuItems.map((item) => (
              <li key={item.id}>
                <button
                  ...
                  className={`w-full flex items-center space-x-4 p-3 rounded-xl transition-all duration-200 group ${currentView === item.id
                    ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/40'
                    : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'
                    }`}
                >
                  <div className={`transition-transform duration-200 ${currentView === item.id ? 'scale-110' : 'group-hover:scale-105'}`}>
